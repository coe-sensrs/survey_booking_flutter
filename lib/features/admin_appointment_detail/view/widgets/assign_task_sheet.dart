...
class _AssignTaskSheetState extends ConsumerState<AssignTaskSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // Pop the sheet first so the snackbar renders on the parent Scaffold,
      // not behind the modal barrier.
      if (mounted && context.canPop()) context.pop();

      AppSnackbar.showGlobalSuccess(
        title: 'Task Assigned',
        message:
            'Fieldwork task assigned to ${_selectedMember!.fullName} successfully.',
      );
    } catch (e) {
      // Always dismiss the sheet so the error snackbar is fully visible.
      if (mounted && context.canPop()) context.pop();

      final message = e is Failure ? e.message : e.toString();
      AppSnackbar.showGlobalError(title: 'Assignment Failed', message: message);
    }
    // No finally setState — widget is popped and may be unmounted already.
  }
  ...
  Widget build(BuildContext context) {
    ...
    // PopScope prevents accidental back-swipe dismissal mid-submission.
    return PopScope(
