...
| `submitAppointment` | Applicant | `{ appointmentId?, applicantName, applicantOrgName?, applicantEmail, surveyType, customSurveyName?, state, district, xenDetails, areaName, kmlFile (uploadedAt: ISO string), preferredDate (ISO string), logistics, permissionDocuments[] (uploadedAt: ISO string) }` | Executes inside atomic Firestore transaction: validates verified applicant, enforces 3-booking active quota (`pendingCount < 3`), increments `pendingCount`. If client provides pre-generated `appointmentId`, binds `appointmentRef = db.collection('appointments').doc(data.appointmentId.trim())` so Storage paths match Firestore document ID. Explicitly converts all received ISO-8601 date strings (`preferredDate`, `kmlFile.uploadedAt`, `permissionDocuments[].uploadedAt`) to native Firestore `Timestamp` objects (`Timestamp.fromDate(new Date(...))`), writes appointment with initial status `pending_assignment`, writes initial auditLog entry. **Returns:** `{ appointmentId }`. | `region: "us-central1"`, `cors: true`, `invoker: "public"`, `enforceAppCheck: false`. |
